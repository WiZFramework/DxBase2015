...
namespace basedx11{
	...
	Player::~Player(){
		...
		wstring PlayerPosStr;
		auto Pos = Ptr->GetPosition();
		PlayerPosStr += L"Player,";
		...
		if (m_PlayerCsv.GetRowCount() < 1){
			m_PlayerCsv.AddRow(PlayerPosStr);
		}
		else{
			m_PlayerCsv.UpdateRow(0, PlayerPosStr, false);
		}
		m_PlayerCsv.SaveCsv(false);
	}
