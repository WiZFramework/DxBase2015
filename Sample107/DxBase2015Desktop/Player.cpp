...
namespace basedx11{
	...
	void Player::Create(){
		...
		//ステートマシンの構築
		m_MyStateMachine = make_shared< MyStateMachine<Player> >(GetThis<Player>());
		//最初のステートをDefaultStateに設定
		m_MyStateMachine->SetCurrentState(DefaultState::Instance());
		//DefaultStateの初期化実行を行う
		m_MyStateMachine->GetCurrentState()->Enter(GetThis<Player>());
	}
	...
	void Player::Update2(){
		...
		if (ColPtr->GetHitObject() && GetMyStateMachine()->GetCurrentState() == JumpState::Instance()){
			GetMyStateMachine()->ChangeState(DefaultState::Instance());
		}
	...
	//--------------------------------------------------------------------------------------
	//	class DefaultState : public MyObjState<Player>;
	//	用途: 通常移動
	//--------------------------------------------------------------------------------------
	//ステートのインスタンス取得
	...
	void DefaultState::Execute(const shared_ptr<Player>& Obj){
		...
		if (Obj->IsJumpMotion()){
			//Jumpボタンでステート変更
			Obj->GetMyStateMachine()->ChangeState(JumpState::Instance());
		}
	}
	//ステートにから抜けるときに呼ばれる関数
	...
	//--------------------------------------------------------------------------------------
	//	class JumpState : public MyObjState<Player>;
	//	用途: ジャンプ状態
	//--------------------------------------------------------------------------------------
	//ステートのインスタンス取得
	...
	void JumpState::Execute(const shared_ptr<Player>& Obj){
		...
		if (Obj->JumpMoveMotion()){
			//通常状態に戻る
			Obj->GetMyStateMachine()->ChangeState(DefaultState::Instance());
		}
	}
	//ステートにから抜けるときに呼ばれる関数
