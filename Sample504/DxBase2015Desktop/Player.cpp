...
namespace basedx11{
	...
	void Player::ActionWithBox(const shared_ptr<FixedBox>& FBox){
		...
		if (PtrSpark){
			PtrSpark->InsertSpark(GetComponent<Transform>()->GetPosition(), Color4(1.0f, 0.0f, 0.0f, 1.0f));
		}
	}
	void Player::ActionWithBox(const shared_ptr<MoveBox>& MBox){
		auto PtrSpark = GetStage()->GetSharedGameObject<MultiSpark>(L"MultiSpark", false);
		if (PtrSpark){
			PtrSpark->InsertSpark(GetComponent<Transform>()->GetPosition(),Color4(0.0f,1.0f,0.0f,1.0f));
		}
	}
	...
	void Player::Update2(){
		...
		if (ColPtr->GetHitObject()){
			...
			auto IAssign = dynamic_pointer_cast<AssignInterface>(ColPtr->GetHitObject());
			if (IAssign){
				//インターフェイスを保持してた
				//処理を振り分ける
